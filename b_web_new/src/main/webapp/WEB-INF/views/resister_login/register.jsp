    ...
    function valSendPhone(){
		    ...
		    if (reg.test(phone)){  
	            $.ajax({
				    async: false,
				    type: "post",
					url: '${ctx}/baseInfo/valdatePhone',
					data: "phone=" + strEnc(phone,'${key}'),
   ...
   $("#photoYZM").live("focus", function () {
    ...
    //验证邀请码
 	function valInvitedCode(_invitedCode){
 		...
 		$.ajax({
            type: "post",
            url: "${ctx}/baseInfo/valInvitedCode",
            async: false,
...
function sendMobileCheckCode() {
        ...
        $.ajax({
             ...
             data: "messageType=005&phone="+strEnc(Mobile,'${key}')+"&code="+photoYZM+"&codeType=registerCode&timeOut="+timeOut,
    ...
    function Send_Submit() {
        ...
        if(""==photoYZM||photoYZM==null){
           ...
           return;
		        ...
		        $.ajax({
		            type: "post",
		            url: "${ctx}/baseInfo/userRegister",
		            async: false,
		            data: "mobilePhone="+strEnc(Mobile,'${key}')+"&code="+code+"&password="+strEnc(Password,'${key}')+"&invitedCode="+invitedCode,
		            success: function (msg){
		            	//石亚宁修改
		            	//msg.success=="5"
		                if(msg.success=="5"){ 
        ...
        <div class="ui-content register-box reg-bg2">
					<form class="ui-form ui-form-setpwd reg-box" method="post" action="">
                        ...
                        <div class="ui-form-item reg-form" id="photoYZMS" style="display:hidden">
						    ...
						    <img style="float:left; display:block; margin:10px 0 0 25px;cursor: pointer;" id="imgObj" title="看不清?请点击" style="cursor: pointer;" onclick="changeImg()" src="${ctx}/authimg?codeType=registerCode"/>
						...
						<div class="ui-form-item">
							<div style="margin-left: 98px">
								...
								<span class="lg-form-other"	style="margin-left: 20px;color:black;">已有账号， 请 <a 	title="登 录" href="${ctx}/baseInfo/login" class="textlink" style="color: red">登录</a> </span>
							</div>
						</div>
					</form>
					<div class="blackBg">
