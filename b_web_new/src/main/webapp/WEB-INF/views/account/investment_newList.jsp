...
function GoSearch(flag, pi) {
    ...
    if(""==$("#investType").val()||undefined==$("#investType").val()){
      submitData.condition.state = '';
    }else{
      submitData.condition.state = $("#investType").val();
    }
    //查询条件 结束
    RenderTmpl({
            Data: submitData,
            ...
            AfterFn:function (result) {
                ...
                if (result.totalCount < 1) {
                    $("#myproject-list").html('<div class="can-not-find0" style="background:url(${resource_dir}/images/empty_box.jpg) no-repeat center; text-align:center; font-size:18px; line-height:24px; padding-top:200px;"><p>暂无投资记录</p></div>');
                    $("#pagerHolder").html("");
                    $(".table_head").hide();
		...
		<div class="ui-main">
			...
			<div class="ui-plate-main">
				<div class="mod-contnet">
					...
					<!--项目列表-->
					<div class="myproject-list" id="myproject-list">
					</div>
							<!-- 分页 -->
					<div id="pagerHolder" class="mod-page">
					</div>
				</div>
			</div>
		</div>
...
<script id="tmpl_projectList" type="text/x-jquery-tmpl">
 {{each(i,v) Data.ResultList}}
                     <div class="records-list">
									<div class="title">
										<strong>{{= v.item_prefix}}{{= v.item_name}}</strong>
                                        ...
                                        <a href="${ctx}/account/investRecordDetail?orderId={{= v.ID}}&itemId={{= v[1]}}">投资详情&gt;&gt;</a>
									</div>
									<div class="content">
										<ul>
											<li>
												投资本金：
												<b class="blue" style="color:#666;">{{html getPrHtml2(v)}}&nbsp;元</b>
											</li>
											<li>
												预期收益：
												<b class="red" style="color:#666;">{{= v.income}}&nbsp;元</b>
											</li>
											<li>
												项目起息日：{{= v.startDate}}
											</li>
										...
										<div class="stauts stauts-wait" style="left:440px;">
											 {{html getProjectProgress(v)}}
										</div>
...
function getProjectProgress(p) {
   ...
   /*
      var itemEndXi = p[11];// 项目结束时间  
      var itemEndXiTime = new Date(Date.parse(itemEndXi.replace(/-/g,   "/")));  // 订单创建时间
      var itemEndXiNum=EndTime.getTime()-itemEndXiTime.getTime();
      itemEndXiNum=itemEndXiNum/1000/60/60/24;
      if(itemEndXiNum<2){
      	progress="正在兑付";
      }else{
      	 progress="已成功兑付";
   ...
   */   
   progress="<p style='font-size:14px;'>已兑付，3个工作日内到账</p>";
   }else if(order=="08"){
